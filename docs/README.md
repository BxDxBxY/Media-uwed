...
| `npm run check:publish` | 22 assertions on the publish step against a **database**: article fields in all three languages, source attribution, category handling, no double-publishing, and **no Telegram send when the integration is off**. Refuses to run if `sendOnPublish` is enabled, so it cannot post to a real channel |
| `npm run check:routes` | 37 authorization assertions against a **running** server: every content mutation, admin endpoint and cron route must refuse anonymous callers. Not part of `npm run check` — needs a server and a database |
